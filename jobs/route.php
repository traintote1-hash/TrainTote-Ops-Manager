...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($action === 'add') {
            ...
            $stmt = $pdo->prepare('UPDATE job_route_stops SET outbound_load_status=?,inbound_load_status=?,pull_destination_mode=?,pull_destination_industry_id=?,replacement_source_mode=?,replacement_source_industry_id=? WHERE id=? AND job_id=? AND railroad_id=?');
            $stmt->execute([$outbound,$inbound,$pullMode,$pullId?:null,$sourceMode,$sourceId?:null,$stopId,$jobId,$railroadId]);
