...
function normalizeOperationsText(?string $value): string
{
...
function getCarServiceCategory(array $car): ?string
{
    $equipmentType = normalizeOperationsText($car['equipment_type'] ?? '');

    if (
        strpos($equipmentType, 'covered hopper') !== false
        || strpos($equipmentType, 'open hopper') !== false
        || $equipmentType === 'hopper'
    ) {
        return 'hopper_bulk';
    }

    if (strpos($equipmentType, 'gondola') !== false) {
        return 'gondola_bulk_scrap';
    }

    if (strpos($equipmentType, 'tank') !== false) {
        return 'tank_liquid';
    }

    if (
        strpos($equipmentType, 'boxcar') !== false
        || strpos($equipmentType, 'refrigerator') !== false
        || strpos($equipmentType, 'reefer') !== false
    ) {
        return 'boxcar_general';
    }

    if (
        strpos($equipmentType, 'flatcar') !== false
        || strpos($equipmentType, 'flat car') !== false
        || strpos($equipmentType, 'bulkhead') !== false
        || strpos($equipmentType, 'centerbeam') !== false
    ) {
        return 'flatcar_lumber_heavy';
    }

    return null;
}

function industryMatchesServiceCategory(array $industry, ?string $category): bool
{
    if ($category === null) {
        return false;
    }

    $industryText = normalizeOperationsText(
    ...
    foreach ($keywords[$category] ?? [] as $keyword) {
        if (strpos($industryText, normalizeOperationsText($keyword)) !== false) {
            return true;
        }
    }
...
{
    $serviceCategory = getCarServiceCategory($car);

    return array_values(array_filter(
        $industries,
        fn($industry) =>
            (int)$industry['id'] !== (int)$car['current_industry_id']
            && industryMatchesServiceCategory($industry, $serviceCategory)
    ));
}
...
if (
    ...
    foreach ($eligibleCars as $car) {
        ...
        $destinationOptions = getCompatibleDestinations(
            $industries,
            $car
        );
...
<div class="tt-session-start-page">
    ...
    <div class="alert alert-warning tt-session-alert">
        <strong>No compatible car moves available.</strong>
        <span>Place cars at compatible industries or add industries that match car service types.</span>
    </div>
