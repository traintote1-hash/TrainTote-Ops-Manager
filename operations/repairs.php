...
<?php else: ?>
<div class="card"><div class="table-responsive tt-mobile-cards"><table class="table align-middle mb-0 tt-repair-table"><thead><tr><th>Equipment</th><th>Reported</th><th>Status</th><th>Bad-order details</th><th>Current location</th><th>Origin</th><th></th></tr></thead><tbody>
...
<tr><td data-label="Equipment"><strong><?=ttHtml($identity)?></strong><br><small><?=ttHtml($repair['equipment_type']?:'Not recorded')?></small></td><td data-label="Reported"><?=ttHtml($repair['reported_at']?:'Not recorded')?></td><td data-label="Status"><span class="badge tt-repair-status-<?=ttHtml($repair['status'])?>"><?=ttHtml(ttRepairStatusLabel($repair['status']))?></span></td><td data-label="Bad-order details"><strong>Bad Order</strong><br><small><?=ttHtml($repair['repair_notes_summary']?:($repair['original_notes']?:'Not recorded'))?></small></td><td data-label="Current location"><?=ttHtml(($repair['industry_name']?:'Not recorded').($repair['current_track']!==''?' / '.$repair['current_track']:''))?></td><td data-label="Origin"><?=ttHtml($origin?implode(' / ',$origin):'Not recorded')?></td><td data-label="Action"><a class="btn btn-sm btn-outline-primary tt-mobile-full" href="repair.php?id=<?=(int)$repair['id']?>"><?=$view==='closed'?'View':'View / Edit'?></a></td></tr>
<?php endforeach; ?>
</tbody></table></div></div>
<?php endif; ?>
