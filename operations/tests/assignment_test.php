...
assignmentExpect(strpos($session,"COALESCE(jop.work_scope,'entire_railroad') work_scope")!==false&&strpos($edit,"COALESCE(jop.work_scope,'entire_railroad') work_scope")!==false,'Create and Edit must load each Job Title work scope.');
assignmentExpect(strpos($session,'data-pattern-label')!==false&&strpos($session,'data-scope-label')!==false&&strpos($edit,'data-pattern-label')!==false&&strpos($edit,'data-scope-label')!==false&&strpos($formJs,'refreshJobSummary')!==false,'Create and Edit must show the selected Job Title pattern, scope, and description without leaving the assignment form.');
...
assignmentExpect(strpos($session,'method="post" action="generate.php"')!==false&&strpos($session,'href="generate.php?assignment_id=')===false,'Session Generate and Regenerate controls must build immediately with a CSRF-protected POST.');
assignmentExpect(strpos($generate,"\$_SERVER['REQUEST_METHOD']!=='POST'")!==false&&strpos($generate,"header('Location: '.(\$latestId>0?'work_order.php?id='")!==false,'Direct GET requests must bypass the extra generation page.');
...
assignmentExpect(strpos($workOrder,'$canRegenerate=$canApprove')!==false&&strpos($workOrder,'action="generate.php"')!==false&&strpos($workOrder,'Regenerate Switch List')!==false,'The latest unapproved Work Order must offer direct regeneration.');
assignmentExpect(strpos($workOrder,'Assigned Locomotives')<strpos($workOrder,'tt-work-order-moves')&&strpos($workOrder,'Assigned Locomotives')<strpos($workOrder,'tt-work-order-details'),'Assigned locomotives must render above the work list instead of inside collapsed Details.');
