...
try{
    ...
    if(!$state||$state['session_status']!=='in_progress'||!in_array($state['assignment_status'],['ready','in_progress'],true)||!in_array($state['list_status'],['approved','in_progress'],true))throw new RuntimeException('Switch-list progress can only be changed while the operating session is Active.');
