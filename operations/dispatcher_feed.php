...
try{
 $userId=(int)$_SESSION['user_id'];$railroad=ttDispatcherAccessRailroad($pdo,$userId);ttDispatcherRequireAccess($railroad);$railroadId=(int)$railroad['id'];ttOperationsRequireModule($pdo,$railroadId,'dispatcher');if(($railroad['operations_role']??'')==='dispatcher')ttOperationsRequireModule($pdo,$railroadId,'advanced_roles');$sessionId=(int)($_GET['session_id']??$_POST['session_id']??0);$session=ttDispatcherSession($pdo,$sessionId,$railroadId);
